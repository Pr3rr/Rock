...
if (isnil "IL_Variables") then
{
	...
	IL_Supported_Vehicles_All =  IL_Supported_Vehicles_MOHAWK + IL_Supported_Vehicles_KAMAZ + IL_Supported_Vehicles_TEMPEST + IL_Supported_Vehicles_HEMTT + IL_Supported_Vehicles_VAN + IL_Supported_Vehicles_OFFROAD;
	...
	IL_Supported_Veh_Ammo = ["Land_CargoBox_V1_F"];																		// 2 slots
	IL_Supported_box = ["Box_IND_AmmoVeh_F"];																		// 3 slots
	IL_Supported_Rubberboat = ["B_Boat_Transport_01_F", "C_Rubberboat"];
	IL_Supported_SDV = ["B_SDV_01_F"]; 
	IL_Supported_Cargo20 = ["Land_Cargo20_military_green_F"];   // 6 slots
	...
	IL_Supported_Cargo_NonVeh_HEMTT = IL_Supported_Cargo_NonVeh_Kamaz + IL_Supported_Cargo20 + IL_Supported_box;
...
//	END IL_Init_Veh
	...
	IL_Init_Box =
	{
		...
		if (_obj_type in IL_Supported_box) then
		{
			_obj setVariable["slots", 3, true];
			_obj setVariable["cargo_offset", 1.3, true];//HEMETT Box ~2.09281
			//LOCAL
			//_obj setMass 2400;
			[_obj, 2400] call IL_SetNewMass;
		};
		...
		{
			{
				...
				{
					[_v, "Diese Fracht ist zu groß"] call IL_Vehicle_Chat;
				}
				else
				{
					if (IL_DevMod) then
					...
					{
						[_v, Format ["Ladevorgang von ""%1"" auf ""%2"" beginnt", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName")]] call IL_Vehicle_Chat;
						_done = true;
						_counter = (_box_num);
						...
						{
							[_v, Format ["%1 wurde erfolgreich auf den %2 aufgeladen. Freie Plätze: %3", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName"), abs(_slot_num - _counter)], true] call IL_Vehicle_Chat;
						}
						else
						{
							[_v, Format ["""%1"" Aufgeladen ""%2"" ist nun voll!", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName")], true] call IL_Vehicle_Chat;
						};
		...
		{
			{
				...
				if (_x getVariable "doors" == _doors) then
				{
					...
					{
						[_v, Format ["%1 wird von %2 abgeladen !VORSICHT!", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName")]] call IL_Vehicle_Chat;
						_done = true;
						_skip = false;
						_zload = (_v getVariable "zload") + (_x getVariable "zload_cargo");
						_cargo_offset = (_v getVariable "load_offset") + (_x getVariable "cargo_offset");
						...
						_v allowDamage true;
						uisleep 2;
						[_v, Format ["wurde erfolgreich von  %2  abgeladen. Freie Plätze: %3", getText(configFile >> "cfgVehicles" >> typeOf _x >> "displayName"), getText(configFile >> "cfgVehicles" >> typeOf _v >> "displayName"), _free_slots], true] call IL_Vehicle_Chat;
